...
function phaseLabel(phase: AgentPhase): string {
  if (phase?.kind === "running_tools") {
    ...
    if (names.length === 1) return `${t("Running tool...")} ${names[0]}`;
    if (names.length <= 3) return `${t("Running tool...")} ${names.join(", ")}`;
    return `${t("Running tool...")} ${names.slice(0, 2).join(", ")} (+${names.length - 2})`;
  }
...
const TYPEWRITER_PHRASES = [
  ...
  t("ask me anything."),
  t("let's build something cool."),
  t("explore your codebase."),
  t("draft an email."),
  t("summarize that paper."),
  t("plan your weekend."),
  t("explain it like I'm five."),
  t("pair-program with me."),
  t("fix that pesky bug."),
  t("translate to 中文."),
  t("write a haiku."),
  t("brainstorm ideas."),
  t("review my pull request."),
  t("what should we cook tonight?"),
  t("ship it."),
  t("make it pretty."),
  t("rubber-duck with me."),
];
